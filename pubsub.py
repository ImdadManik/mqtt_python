...
def connectedPublishMsg():
    ...
    IsHeartBeatPublished = True if device_id is not None else False
...
def on_message(client, userdata, msg):
    ...
    if msg.topic == sub_setting_Topic and msg.payload is not None:
        ...
        if msg.payload != last_retain_message:
            ...
            device_status = bool(payload_dict.get('DeviceStatus', device_status))
            account_status = bool(payload_dict.get('AccountStatus', account_status))
    ...
    #             ldr_value = bool(payload_dict.get('LDR', ldr_value))
    #             ldr_alert_freq = payload_dict.get('LDRAlertFreq', ldr_alert_freq)
    #             device_status = bool(payload_dict.get('Status', device_status))
    #             door_value = bool(payload_dict.get('Door', door_value))
    #             pir_value = bool(payload_dict.get('PIR', pir_value))
    #             account_id = payload_dict.get('AccountId', account_id)
    #             device_id =  payload_dict.get('Id', device_id)
    ...
    #             #print("")
    #             #print("---------------------------------------------------------------------------------------------------------")
    #             #print(f"temp value : {temp_value} | TempAlertFreq: {temp_alert_freq} | ldr value : {ldr_value} | LDRAlertFreq : {ldr_alert_freq} | Status: {device_status} | Door: {door_value} | PIR: {pir_value} | AccountId: {account_id} | DeviceId: {device_id}")
    #             #print("---------------------------------------------------------------------------------------------------------")
    ...
    #         device_status = bool(payload_data.get('Status', device_status))
                    ...
                    if device_status and account_status:
                        if temp.getTemperature() is not None:
                            if temp_value is True and temp_alert_freq and (temp.getTemperature() > min_temp_alert):
                                if time.time() - start_time_temp >= (temp_alert_freq - 3) * 60:
                        ...
                        if ldr_value is True and ldr_alert_freq:
                                    if time.time() - start_time_ldr >= (ldr_alert_freq - 3) * 60:
